  ...
  return (
    <div className="bg-white my-2 px-4 md:px-16 lg:px-24">
      <div className="container mx-auto py-4 flex flex-col md:flex-row space-x-2">
        ...
        {slides.map((item)=>(
           <img src={item} alt="img" className="w-full h-full bg-center bg-cover transition ease-in-out duration-500" />
        ))}
        <div className="absolute top-0 w-full h-full flex items-center justify-between px-8">
        <button onClick={()=> previousSlide()} className="hidden group-hover:block  text-2xl text-white rounded-full p-2 bg-black/20 cursor-pointer"><BsChevronCompactLeft size={30} /></button>
        <button onClick={()=> nextSlide()} className="hidden group-hover:block  text-2xl text-white rounded-full p-2 bg-black/20 cursor-pointer"><BsChevronCompactRight size={30} /></button>
        </div>
